...
from .load_robot import JointConfig, JointInfo, generate_joints_config, generate_xml, group_to_stl, load_robot_assemblies, load_assemby_json, render_tree, split_by_group, download_all_parts
import graphviz
...
def main():
    ...
    if config['outputFormat'] == 'urdf':
        robot = RobotDescription(config['robotName'])
    ...
    assembly = load_assemby_json()

    download_all_parts(assembly)

    assemblies = load_robot_assemblies(assembly)

    groups_by_sub = split_by_group(assemblies, assembly)

    # for key, groups in groups_by_sub.items():
    #     for idx, group in enumerate(groups):
    #         group_to_stl(key, idx, group)

    joints = generate_joints_config(groups_by_sub, assembly)
   
    cont = True
    while cont:
        render_tree(joints)
        inp = int(input('Enter index of joint to invert or -1 to end: '))
        if inp != -1:
            joints[inp] = JointConfig(
                joints[inp].child_key, joints[inp].child_group_idx, joints[inp].child_transform,
                joints[inp].parent_key, joints[inp].parent_group_idx, joints[inp].parent_transform,
                JointInfo(
                    joints[inp].info.childCs,
                    joints[inp].info.parentCs
                ),
                joints[inp].type)
        else:
            cont = False
        
    generate_xml(groups_by_sub, joints, robot)

    # Start building the robot
    # buildRobot(robot, np.matrix(np.identity(4)))
    # robot.finalize()
    # print(tree)
